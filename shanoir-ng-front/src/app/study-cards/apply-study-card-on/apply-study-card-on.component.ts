...
export class ApplyStudyCardOnComponent implements OnInit {
    ...
    ngOnInit(): void {
        const studyCardId: number = parseInt(
            this.activatedRoute.snapshot.paramMap.get("id"),
        );
        const datasetIds: number[] = Array.from(
        ...
        if (datasetIds?.length > 0) {
            acquisitionPromise =
                this.datasetAcquisitionService.getAllForDatasets(datasetIds);
            studycardPromise = this.studycardService.getAll();
        } else if (!Number.isNaN(studyCardId)) {
            acquisitionPromise =
                this.datasetAcquisitionService.getByStudycardId(studyCardId);
        ...
        const rightsPromise: Promise<Map<number, StudyUserRight[]>> =
            this.studyRightsService
                .getMyRights()
                .then((rights) => (this.studyRights = rights));

        this.status = this.breadcrumbsService.currentStep.data.status
            ? this.breadcrumbsService.currentStep.data.status
            : "default";
        if (
            this.breadcrumbsService.currentStep.data.showIncompatibles !=
            undefined
        ) {
            this.showIncompatibles =
                this.breadcrumbsService.currentStep.data.showIncompatibles;
        }

        const filteredAcquisitionsPromise: Promise<DatasetAcquisition[]> =
            Promise.all([acquisitionPromise, rightsPromise]).then(
                ([acquisitions, rights]) => {
                    const nonAdminAcquisitions: DatasetAcquisition[] =
                    ...
                    this.browserPaging = new BrowserPaging(
                        this.datasetAcquisitions,
                        this.columnsDefs,
                    );
                    this.selectAll();
        ...
        Promise.all([filteredAcquisitionsPromise, studycardPromise]).then(
            ([, studycards]) => {
                if (Array.isArray(studycards)) {
                    this.studyCards = studycards;
                    this.updateOptions();
                    if (this.breadcrumbsService.currentStep.data.studyCardId) {
                        this.studycard = this.studyCards.find(
                            (sc) =>
                                sc.id ==
                                this.breadcrumbsService.currentStep.data
                                    .studyCardId,
                        );
                    }
                } else {
    ...
    selectAll() {
        this.selectedAcquisitionIds = new Set(
            this.datasetAcquisitions.map((acq) => acq.id),
        );
                        ...
                        .catch((error) => {
                            ...
                            if (
                                error?.error?.message?.includes(
                                    "could not get dicom attributes from pacs",
                                )
                            ) {
                                this.errorMessage =
                                    "Could not get dicom attributes from pacs";
        ...
        const colDef: ColumnDefinition[] = [
            {
                ...
                type: "boolean",
                cellRenderer: (row) =>
                    this.isCompatible(row.data.acquisitionEquipment?.id),
                awesome: "fa-solid fa-circle",
                awesomeFalse: "fa-solid fa-triangle-exclamation",
                color: "green",
                colorFalse: "orangered",
                disableSorting: true,
            },
            {
                headerName: "Id",
            ...
            {
                headerName: "Center Equipment",
                field: "acquisitionEquipment",
                orderBy: ["acquisitionEquipmentId"],
                cellRenderer: (params: any) =>
                    this.transformAcqEq(params.data?.acquisitionEquipment),
                route: (dsAcq: DatasetAcquisition) =>
                    "/acquisition-equipment/details/" +
                    dsAcq?.acquisitionEquipment?.id,
            },
            {
                headerName: "Study",
                field: "examination.study.name",
                defaultField: "examination.study.id",
                orderBy: ["examination.studyId"],
                route: (dsAcq: DatasetAcquisition) =>
                    "/study/details/" + dsAcq?.examination?.study?.id,
            },
            {
                headerName: "Examination date",
                type: "date",
                field: "examination.examinationDate",
            },
            {
                ...
                disableSorting: true,
                route: (dsAcq: DatasetAcquisition) =>
                    dsAcq?.acquisitionEquipment?.center
                        ? "/center/details/" +
                          dsAcq?.acquisitionEquipment?.center?.id
                        : null,
            },
            { headerName: "Last StudyCard", field: "studyCard.name" },
            {
                ...
                action: (item) =>
                    this.router.navigate([
                        "/dataset-acquisition/details/" + item.id,
                    ]),
            },
        ];
        return colDef;
        ...
        if (!acqEqpt) return "";
            ...
            return (
                ...
                (manufModel.magneticField
                    ...
                    : "") +
                " (" +
                DatasetModalityType.getLabel(manufModel.datasetModalityType) +
                ") " +
    ...
    updateNbIncompatible() {
        ...
        if (this.datasetAcquisitions && this.studycard) {
            this.datasetAcquisitions.forEach((acq) => {
                if (
                    this.selectedAcquisitionIds.has(acq.id) &&
                    acq.acquisitionEquipment?.id !=
                        this.studycard.acquisitionEquipment?.id
                ) {
                    this.nbIncompatible++;
                }
            });
    ...
    updateStudyCard() {
        ...
        this.breadcrumbsService.currentStep.data.studyCardId =
            this.studycard?.id;
    }
        ...
        if (this.studyCards) {
            ...
            this.studyCards.forEach((sc) => {
                if (sc) {
                    ...
                    option.compatible =
                        this.datasetAcquisitions.findIndex(
                            (acq) =>
                                acq.acquisitionEquipment?.id !=
                                sc.acquisitionEquipment?.id,
                        ) == -1;
