    ...
    private buildForm(): UntypedFormGroup {
        ...
        formGroup.controls.from.setValidators([
            ...
            (control: AbstractControl) =>
                Validators.max(formGroup.get("to").value)(control),
        ]);
        formGroup.controls.to.setValidators([
            Validators.required,
            (control: AbstractControl) =>
                Validators.min(formGroup.get("from").value)(control),
        ]);
        if (this.nbExaminations) {
            formGroup.controls.to.addValidators([
                Validators.max(this.nbExaminations),
            ]);
        }
        return formGroup;
    }
