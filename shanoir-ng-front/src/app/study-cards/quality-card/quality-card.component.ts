...
export class QualityCardComponent extends EntityComponent<QualityCard> {
    ...
    get qualityCard(): QualityCard {
        return this.entity;
    }
    set qualityCard(qc: QualityCard) {
        this.entity = qc;
    }

    initView(): Promise<void> {
        this.hasAdministrateRightPromise = this.hasAdminRightsOnStudy().then(
            (res) => (this.isStudyAdmin = res),
        );
    ...
    initCreate(): Promise<void> {
        ...
        this.fetchStudies().then(() => {
            if (this.forceStudyId) {
                this.lockStudy = true;
                this.qualityCard.study = this.studies.find(
                    (st) => st.id == this.forceStudyId,
                ) as unknown as Study;
                this.onStudyChange();
            }
        });
    ...
    buildForm(): FormGroup {
        const form: FormGroup = this.formBuilder.group({
            ...
            toCheckAtImport: [
                this.qualityCard.toCheckAtImport,
                [Validators.required],
            ],
            rules: [
                this.qualityCard.rules,
                [StudyCardRulesComponent.validator],
            ],
    ...
    private hasAdminRightsOnStudy(): Promise<boolean> {
        if (this.keycloakService.isUserAdmin()) {
            ...
            return this.studyRightsService
                .getMyRightsForStudy(this.qualityCard.study.id)
                .then((rights) => {
            ...
            .then((accept) => {
                if (accept) {
                    ...
                    this.qualityCardService
                        .applyOnStudy(this.qualityCard.id)
                        .then((result) => {
    ...
    test() {
        ...
        this.examinationService
            .findExaminationIdsByStudy(this.qualityCard.study.id)
            .then((examIds) => {
                this.nbExaminations = examIds.length;
                if (examIds?.length > 0) {
                    if (examIds.length > this.NB_EXAM_THRESHOLD) {
                        this.openSetTestInterval(this.nbExaminations)
                            .then((response) => {
                                if (!response) {
                                    this.performTest();
                                } else if (response instanceof Interval) {
                                    this.performTest(
                                        (response as Interval).from,
                                        (response as Interval).to,
                                    );
                                }
                            })
                            .finally(() => {
    ...
    openSetTestInterval(nbExaminations: number): Promise<Interval | "cancel"> {
        const modalRef: ComponentRef<TestQualityCardOptionsComponent> =
    ...
    performTest(start?: number, stop?: number) {
        this.qualityCardService
            .testOnStudy(this.qualityCard.id, start, stop)
            .then((result) => {
    ...
    static downloadReport(
        report: { columnDefs: any[]; items: any[] },
        name?: string,
    ) {
        if (!report) return;
        let csvStr: string = "";
        ...
        for (const entry of report.items) {
            csvStr +=
                ...
                report.columnDefs
                    .map(
                        (col) =>
                            '"' + TableComponent.getCellValue(entry, col) + '"',
                    )
                    .join(",");
        }
    ...
    protected downloadReport() {
        QualityCardComponent.downloadReport(
            this.report,
            this.qualityCard?.name,
        );
    }
