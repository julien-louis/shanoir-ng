...
export class StudyCardRuleComponent implements OnChanges {
    ...
    @ViewChildren(StudyCardActionComponent)
    assignmentChildren: QueryList<StudyCardActionComponent>;
            ...
            this.rulePromise.then(() => {
                if (this.assignmentFields) {
                    this.assignmentFieldOptions = this.assignmentFields
                        .filter(
                            (field) =>
                                this.rule.scope == "DatasetAcquisition" ||
                                (this.rule.scope == "Dataset" &&
                                    field.scope == "Dataset"),
                        )
                        .map(
                            (field) =>
    ...
    addNewCondition() {
        const cond = new StudyCardCondition(
    ...
    deleteAction(index: number) {
        const fieldOption: Option<string> = this.assignmentFieldOptions.find(
            (opt) => opt.value === this.rule.assignments[index].field,
        );
