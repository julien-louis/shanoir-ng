    ...
    implements OnChanges, ControlValueAccessor
{
    ...
    @ViewChildren("studyCardRule,qualityCardRule") ruleElements: QueryList<
        StudyCardRuleComponent | QualityCardRuleComponent
    >;
    ...
    private coilOptionsSubject: Subject<Option<Coil>[]> = new BehaviorSubject<
        Option<Coil>[]
    >(null);
    private coilOptions: Observable<Option<Coil>[]> =
        this.coilOptionsSubject.asObservable();
    private allCoilsPromise: SuperPromise<Coil[]> = new SuperPromise();
    @Input() showErrors: boolean = false;
    @Output() importRules: EventEmitter<void> = new EventEmitter();
    @Output() selectedRulesChange: EventEmitter<
        (StudyCardRule | QualityCardRule)[]
    > = new EventEmitter();
    ...
    constructor(
        ...
        if (this.breadcrumbService.currentStep.data.rulesToAnimate)
            this.rulesToAnimate =
                this.breadcrumbService.currentStep.data.rulesToAnimate;
        else
            this.breadcrumbService.currentStep.data.rulesToAnimate =
                this.rulesToAnimate;
    }
    ...
    initFields() {
        this.assignmentFields = [
            new ShanoirMetadataField(
                "Dataset modality type",
                "MODALITY_TYPE",
                "Dataset",
                DatasetModalityType.options,
            ),
            new ShanoirMetadataField(
                "Protocol name",
                "PROTOCOL_NAME",
            ...
            new ShanoirMetadataField(
                ...
                "EXPLORED_ENTITY",
                "Dataset",
                ExploredEntity.options,
            ),
            new ShanoirMetadataField(
                "Acquisition contrast",
                "ACQUISITION_CONTRAST",
                "DatasetAcquisition",
                AcquisitionContrast.options,
            ),
            new ShanoirMetadataField(
                "MR sequence application",
                "MR_SEQUENCE_APPLICATION",
                "DatasetAcquisition",
                MrSequenceApplication.options,
            ),
            new ShanoirMetadataField(
                "MR sequence physics",
                "MR_SEQUENCE_PHYSICS",
                "DatasetAcquisition",
                MrSequencePhysics.options,
            ),
            new ShanoirMetadataField(
                this.cardType == "studycard"
            ...
            new ShanoirMetadataField(
                ...
                "CONTRAST_AGENT_USED",
                "DatasetAcquisition",
                ContrastAgent.options,
            ),
            new ShanoirMetadataField(
                "Mr Dataset Nature",
                "MR_DATASET_NATURE",
                "Dataset",
                MrDatasetNature.options,
            ),
            new ShanoirMetadataField(
                "BIDS data type",
                "BIDS_DATA_TYPE",
                "DatasetAcquisition",
                BidsDataType.options,
            ),
        ];
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes.manufModelId && this.manufModelId) {
            this.allCoilsPromise.then((allCoils) => {
                ...
                allCoils
                    .filter(
                        (coil) =>
                            coil.manufacturerModel.id == this.manufModelId,
                    )
                    .forEach((coil) =>
                        optionArr.push(new Option<Coil>(coil, coil.name)),
                    );
        ...
        if (changes.studyId && this.studyId) {
            this.allCoilsPromise.then((allCoils) => {
                ...
                allCoils
                    .filter((coil) =>
                        coil.center?.studyCenterList?.find(
                            (sc) => sc.study.id == this.studyId,
                        ),
                    )
                    .forEach((coil) =>
                        optionArr.push(new Option<Coil>(coil, coil.name)),
        ...
        if (
            changes.cardType &&
            this.cardType &&
            (!this.assignmentFields || !this.conditionFields)
        ) {
            this.initFields();
        }
    }
    ...
    switchWithFollowing(index: number) {
        ...
        const b =
            this.ruleElements.toArray()[index + 1].elementRef.nativeElement;
    ...
    copy(index: number) {
        const original: StudyCardRule | QualityCardRule = this.rules.slice(
            index,
            index + 1,
        )[0];
        let copy;
            ...
            rules.forEach((rule) => {
                ...
                if (
                    rule.conditions?.find(
                        (cond) =>
                            cond.scope == "StudyCardDICOMConditionOnDatasets" &&
                            !cond.dicomTag,
                    )
                ) {
                ...
                if (
                    rule.conditions?.find(
                        (cond) =>
                            cond.operation != "PRESENT" &&
                            cond.operation != "ABSENT" &&
