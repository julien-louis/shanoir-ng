...
export class AcquisitionEquipmentComponent extends EntityComponent<AcquisitionEquipment> {
    ...
    get acqEquip(): AcquisitionEquipment {
        return this.entity;
    }
    set acqEquip(acqEquip: AcquisitionEquipment) {
        this.entity = acqEquip;
    }
    ...
    constructor(
        ...
        this.fromImport =
            this.router.currentNavigation()?.extras?.state?.fromImport;
    }
    ...
    initEdit(): Promise<void> {
        this.currentManufAndSerialAndCenter =
            new ManufacturerAndSerialAndCenter(
                this.acqEquip.manufacturerModel,
                this.acqEquip.serialNumber,
                this.acqEquip.center,
            );
        this.getManufModels();
        return Promise.all([this.centerService.getCentersNames()]).then(
            ([centers]) => {
    ...
    private prefill() {
        ...
        if (this.fromImport) {
            this.acqEquip.serialNumber =
                this.fromImport.split("-")[2] != "null"
                    ? this.fromImport.split("-")[2]
                    : "";
        }
    }

    private updateAcquEq(): void {
        const mod = DatasetModalityType.all().find(
            (dsMod) =>
                dsMod.toString() ==
                this.acqEquip.manufacturerModel.datasetModalityType,
        );
        if (mod)
            this.datasetModalityTypeStr = DatasetModalityType.getLabel(mod);
    }
        ...
        const form: UntypedFormGroup = this.formBuilder.group(
            {
                serialNumber: [
                    this.acqEquip.serialNumber,
                    [this.noSpacesStartAndEndValidator],
                ],
                manufacturerModel: [
                    this.acqEquip.manufacturerModel,
                    [Validators.required],
    ...
    private noSpacesStartAndEndValidator = (
    ...
    private uniqueEquipmentValidator: AsyncValidatorFn = async (
        form: AbstractControl,
    ): Promise<ValidationErrors | null> => {
        if (!form) return of(null);

        const serialNumber = form.get("serialNumber")?.value as string;
        const manufacturerModel = form.get("manufacturerModel")
            ?.value as ManufacturerModel;
        const center = form.get("center")?.value as Center;

        if (!serialNumber || !manufacturerModel || !center) return null;
        if (
            serialNumber == this.currentManufAndSerialAndCenter?.serial &&
            manufacturerModel.id ==
                this.currentManufAndSerialAndCenter?.manuf.id &&
            center.id == this.currentManufAndSerialAndCenter?.center.id
        )
            return null;

        try {
            if (typeof serialNumber == "string") {
                const exists = await this.acqEquipService.checkDuplicate(
                    serialNumber,
                    manufacturerModel,
                    center,
                );
