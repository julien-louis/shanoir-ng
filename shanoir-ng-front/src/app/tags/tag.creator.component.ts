        ...
        if (this.text != null && this.selectedColor != null) {
            ...
            if (this.model.find((tag) => (tag as Tag).equals(newTag))) {
                this.dialogService.error(
                    "Error",
                    "A tag with this color and name already exist !",
                );
            } else {
                ...
                this.displayedTags.add({
                    tag: newTag,
                    darkFont: isDarkColor(newTag.color),
                });
    ...
    public deleteTag(tag: { tag: Tag; darkFont: boolean }) {
        if (this.tagUsed(tag.tag)) {
            this.dialogService.inform(
                "Cannot delete!",
                "Sorry, this tag is currently linked to one or more subject(s) in this study.",
            );
            return;
        } else {
    ...
    writeValue(obj: any): void {
        ...
        if (this.model) {
            (this.model as Tag[]).forEach((tag) =>
                this.displayedTags.add({
                    tag: tag,
                    darkFont: isDarkColor(tag.color),
                }),
            );
        }
    }
