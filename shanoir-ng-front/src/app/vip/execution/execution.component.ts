...
export class ExecutionComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.datasetsPromise = this.datasetService
            .getByIds(this.processingService.selectedDatasets)
            .then((result) => {
        ...
        this.keycloakService.getRefreshToken().then((refreshToken: string) => {
    ...
    removeDatasetFromParam(event, paramName) {
        this.datasetsByParam[paramName].splice(
            this.datasetsByParam[paramName].indexOf(event),
            1,
        );
    }
    ...
    initExecutionForm() {
        ...
        this.pipeline.parameters.forEach((parameter) => {
            ...
            if (
                ...
                parameter.type != ParameterType.File
            ) {
    ...
    private createColumnDefs() {
        this.pipeline.parameters.forEach((parameter) => {
            if (parameter.type == ParameterType.File) {
                ...
                this.columnDefs[parameter.name].push({
    ...
    onApplyParameters() {
        ...
        this.datasetsPromise.then(() => {
            const availableDatasets: DatasetLight[] = Array.from(
                this.selectedDatasets,
            );

            this.datasetsOptions = [];
            availableDatasets.forEach((dataset) => {
                this.datasetsOptions.push(
                    new Option<DatasetLight>(
                        dataset,
            ...
            this.pipeline.parameters.forEach((parameter) => {
                if (this.isAFile(parameter)) {
                    ...
                    paramDatasets.forEach((dataset) => {
                        availableDatasets.splice(
                            availableDatasets.indexOf(dataset),
                            1,
                        );
                    });
        ...
        this.executionService.createExecution(exec).then(
            (monitoring) => {
                this.router.navigate([
                    `/dataset-processing/details/${monitoring.id}`,
                ]);
            },
            (error) => {
                this.msgService.log(
                    "error",
                    "Sorry, an error occurred while submitting execution.",
                );
                console.error(error);
            },
        );
    ...
    private initExecutionCandidate() {
        ...
        candidate.name = this.cleanProcessingName(
            this.executionForm.get("execution_name").value,
        );
        ...
        this.pipeline.parameters.forEach((parameter) => {
            if (this.isAFile(parameter)) {
                ...
                dto.datasetIds = this.datasetsByParam[parameter.name].map(
                    (dataset) => {
                        return dataset.id;
                    },
                );
                ...
                this.executionForm.get(parameter.name).value?.toString()
            ) {
                candidate.inputParameters[parameter.name] = [
                    this.executionForm.get(parameter.name).value.toString(),
                ];
            }
        });
    ...
    getDefaultExecutionName(): string {
        return this.cleanProcessingName(
            this.pipeline.name +
                "_" +
                this.pipeline.version +
                "_" +
                formatDate(new Date(), "dd-MM-YYYY_HHmmss", "en-US"),
        );
    }
