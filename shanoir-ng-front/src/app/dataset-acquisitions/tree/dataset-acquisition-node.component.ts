    ...
    implements OnChanges
{
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes["input"]) {
            if (this.input instanceof DatasetAcquisitionNode) {
                ...
                const label: string =
                    "Dataset Acquisition n°" + this.input.datasetAcquisition.id;
                this.node = new DatasetAcquisitionNode(
                    this.input.parentNode,
                    this.input.datasetAcquisition.id,
                    label,
                    UNLOADED,
                    this.input.studyRights.includes(
    ...
    loadDatasets() {
        if (this.node.datasets == UNLOADED) {
            this.loading = true;
            this.datasetService
                .getByAcquisitionId(this.node.id)
                .then((datasets) => {
                    this.node.datasets = datasets
                        .map((ds) =>
    ...
    download() {
        ...
        if (!this.datasetIds || this.datasetIds.length == 0) {
            this.consoleService.log(
                "warn",
                "Sorry, no dataset for acquisition n°" + this.node?.id,
            );
        ...
        datasetIdsReady.then(() => {
            this.massDownloadService.downloadAllByAcquisitionId(
                this.node.id,
                this.downloadState,
            );
        });
