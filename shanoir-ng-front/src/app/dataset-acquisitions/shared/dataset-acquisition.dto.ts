...
export class DatasetAcquisitionDTOService {
     ...
     */
    public toDatasetAcquisition(
        dto: DatasetAcquisitionDTO,
        result?: DatasetAcquisition,
    ): Promise<DatasetAcquisition> {
        if (!result)
            result = DatasetAcquisitionUtils.getNewDAInstance(dto.type);
        ...
        promises.push(
            this.studyService
                .get(dto.examination.studyId)
                .then((study) => (result.examination.study = study)),
        );
    ...
    public toDatasetAcquisitions(
            ...
            if ((dto as DatasetAcquisitionDatasetsDTO).datasets) {
                entity.datasets = (
                    ...
                    const simpleDataset: Dataset =
                        DatasetUtils.getDatasetInstance(dsdto.type);
        ...
        return Promise.all([
            ...
            for (const entity of result) {
                ...
                if (entity.examination && entity.examination.study)
                    entity.examination.study = studies.find(
                        (study) => study.id == entity.examination.study.id,
                    );
            }
            return result;
        });
    ...
    static mapSyncFields(
        ...
        if (dto.examination) {
            ...
            ExaminationDTOService.mapSyncFields(
                dto.examination,
                entity.examination,
            );
        }
        switch (entity.type) {
            case "Mr": {
                (entity as MrDatasetAcquisition).protocol = Object.assign(
                    new MrProtocol(),
                    (dto as MrDatasetAcquisitionDTO).protocol,
                );
                break;
            }
            case "Pet": {
                (entity as PetDatasetAcquisition).protocol = Object.assign(
                    new PetProtocol(),
                    (dto as PetDatasetAcquisitionDTO).protocol,
                );
                break;
            }
            case "Ct": {
                (entity as CtDatasetAcquisition).protocol = Object.assign(
                    new CtProtocol(),
                    (dto as CtDatasetAcquisitionDTO).protocol,
                );
                break;
            }
            case "Xa": {
                (entity as XaDatasetAcquisition).protocol = Object.assign(
                    new XaProtocol(),
                    (dto as XaDatasetAcquisitionDTO).protocol,
                );
                break;
            }
        }
...
export class DatasetAcquisitionDTO {
    constructor(dsAcq: DatasetAcquisition) {
        ...
        this.acquisitionEquipmentId = dsAcq.acquisitionEquipment
