...
export class DatasetProcessingComponent extends EntityComponent<DatasetProcessing> {
    protected datasetProcessingTypes: Option<DatasetProcessingType>[] =
        DatasetProcessingType.options;
    ...
    get datasetProcessing(): DatasetProcessing {
        return this.entity;
    }
    set datasetProcessing(datasetProcessing: DatasetProcessing) {
        this.entity = datasetProcessing;
    }
    ...
    initView(): Promise<void> {
        this.studyService
            .get(this.datasetProcessing.studyId, "lazy")
            .then((study) => {
        ...
        this.subscriptions.push(
            this.executionMonitoringService
                .getExecutionMonitoring(this.datasetProcessing.id)
                .subscribe(
    ...
    initEdit(): Promise<void> {
        this.fetchStudies().then(() => {
            const firstDatasetId: number =
                this.datasetProcessing.inputDatasets?.[0]?.id;
            if (firstDatasetId) {
                Promise.all([
                    ...
                    if (dataset) {
                        ...
                        this.fetchDatasets();
                        this.subject = this.subjectOptions?.find(
                            (opt) => opt.value.id == subjectId,
                        )?.value;
                    }
                });
            }
    ...
    askForSubjectChangeConfirmation(): Promise<boolean> {
        if (
            this.datasetProcessing.inputDatasets?.length > 0 ||
            this.datasetProcessing.outputDatasets?.length > 0
        ) {
            return this.confirmDialogService.confirm(
                "Change Subject",
                "Are you sure you want to change the subject for this processing ? Every dataset input and output will be removed from the current lists.",
            );
        } else {
    ...
    askForStudyChangeConfirmation(): Promise<boolean> {
        if (
            ...
            return this.confirmDialogService.confirm(
                "Change Study",
                "Are you sure you want to change the study for this processing ? Every dataset input and output will be removed from the current lists.",
            );
        } else {
            return Promise.resolve(true);
        }
    ...
    fetchStudies(): Promise<void> {
        return this.studyService.getAll().then((studies) => {
            this.studyOptions = studies?.map(
                (study) => new Option<number>(study.id, study.name),
            );
        });
    }
    ...
    get studyName(): string {
        const studyOption = this.studyOptions?.find(
            (opt) => opt.value == this.datasetProcessing?.studyId,
        );
    ...
    fetchSubjects(): Promise<void> {
        ...
        return this.studyService
            .findSubjectsByStudyId(this.datasetProcessing.studyId)
            .then((subjects) => {
    ...
    fetchDatasets(): Promise<void> {
        if (!this.datasetProcessing?.studyId || !this.subject?.id)
            return Promise.resolve();
        return this.datasetService
            .getByStudyIdAndSubjectId(
                this.datasetProcessing.studyId,
                this.subject.id,
            )
            .then((datasets) => {
                for (const dataset of datasets) {
                    this.inputDatasetOptions.push(
                        new Option<Dataset>(dataset, dataset.name),
                    );
                }
            });
    }
    ...
    completeDatasetsDetails(): Promise<void> {
        return this.datasetService
            .getByIds(
                new Set<number>([
                    ...(this.datasetProcessing.inputDatasets?.map(
                        (ds) => ds.id,
                    ) || []),
                    ...(this.datasetProcessing.outputDatasets?.map(
                        (ds) => ds.id,
                    ) || []),
                ]),
            )
            .then((datasets) => {
                const datasetsMap: Map<number, DatasetLight> = new Map<
                    number,
                    DatasetLight
                >();
                for (const dataset of datasets) {
    ...
    buildForm(): UntypedFormGroup {
        const formGroup: UntypedFormGroup = this.formBuilder.group({
            studyId: [
                { value: this.datasetProcessing?.studyId },
                Validators.required,
            ],
            subject: [
                {
                    value: this.subject,
                    disabled: !this.datasetProcessing?.studyId,
                },
                Validators.required,
            ],
            datasetProcessingType: [
                this.datasetProcessing.datasetProcessingType,
                Validators.required,
            ],
            processingDate: [
                this.datasetProcessing.processingDate,
                Validators.required,
            ],
            inputDatasets: [
                {
                    value: this.datasetProcessing.inputDatasets,
            ...
            outputDatasets: [
                {
                    value: this.datasetProcessing.outputDatasets,
                    disabled: !this.subject,
                },
            ],
    ...
    protected prefillProperties(): void {
        ...
        if (this.breadcrumbsService.currentStep.isPrefilled("subject")) {
            this.breadcrumbsService.currentStep
                .getPrefilledValue("subject")
                .then((prefilledSubject) => {
                    this.form.get("subject").disable();
                    this.subjectOptions = [
                        new Option(prefilledSubject.id, prefilledSubject.name),
                    ];
    ...
    public downloadStdout() {
        ...
        this.vipClientService
            .getStdout(this.executionMonitoring.identifier)
            .toPromise()
            .then((response) => {
    ...
    public downloadStderr() {
        ...
        this.vipClientService
            .getStderr(this.executionMonitoring.identifier)
            .toPromise()
            .then((response) => {
                this.downloadLogIntoBrowser(response, filename);
            });
    ...
    public formatDate(millis: number): string {
        return millis
            ? formatDate(new Date(millis), "dd/MM/yyyy HH:mm:ss", "en-US")
            : "";
    }
