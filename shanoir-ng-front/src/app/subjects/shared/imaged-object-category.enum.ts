...
export namespace ImagedObjectCategory {
    ...
    export function toOptions(
        preclinical: boolean = false,
    ): Option<ImagedObjectCategory>[] {
        return all(preclinical).map(
            (prop) => new Option<ImagedObjectCategory>(prop, getLabel(prop)),
        );
    }
    ...
    export function all(preclinical: boolean = false): ImagedObjectCategory[] {
        ...
        if (preclinical) {
            allPreclinicalImagedObjectCategory.push(
