    ...
    implements ControlValueAccessor, OnDestroy, OnChanges, AfterViewInit
{
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (
            ...
            changes.optionArr &&
            !arraysEqual(
                changes.optionArr?.currentValue,
                changes.optionArr?.previousValue,
            )
        ) {
            ...
            changes.optionBuilder &&
            this.optionBuilder &&
            changes.optionBuilder.currentValue !=
                changes.optionBuilder.previousValue
        ) {
            this.searchText = null;
    ...
    private initSelectedOption() {
        if (this.selectedOption || this.inputValue) {
            ...
            if (this.selectedOption) {
                index = this.options.findIndex((eachOpt) =>
                    this.valuesEqual(eachOpt.value, this.selectedOption.value),
                );
            } else {
                index = this.options.findIndex((eachOpt) =>
                    this.valuesEqual(eachOpt.value, this.inputValue),
                );
    ...
    writeValue(value: any): void {
        ...
        if (value && this.options?.length > 0) {
            const index = this.options.findIndex((eachOpt) =>
                this.valuesEqual(eachOpt.value, value),
            );
            if (index > -1) {
    ...
    public get selectedOption(): Option<any> {
        return this.options &&
            ...
            ? this.options[this.selectedOptionIndex]
            : this.inputValue
              ? new Option(this.inputValue, this.getLabel(this.inputValue))
              : null;
    }
    ...
    private computeMinWidth() {
        ...
        if (
            ...
            this.displayableOptions.length > 0 &&
            this.hiddenOption
        ) {
            this.displayableOptions.forEach((opt) => {
                if (
                    ...
                    !opt.label ||
                    opt.label.length >
                        maxOption.label.length - maxOption.label.length * 0.1
                ) {
                    this.hiddenOption.nativeElement.innerText = opt.label;
                    const width: number =
                        this.hiddenOption.nativeElement.offsetWidth;
                    if (width > maxWidth) {
                        maxWidth = width;
                        maxOption = opt;
                    }
    ...
    private set searchText(text: string) {
        ...
        if (!text) {
            ...
            if (this.searchText.length <= 0) {
                ...
                this.filteredOptions.searchText != this.searchText
            ) {
                this.filteredOptions = new FilteredOptions(
                    this.searchText,
                    this.options,
                );
            }
    ...
    private computeDisplayedOptions() {
        if (
            this.firstScrollOptionIndex == undefined ||
        ...
        this.displayedOptions = this.displayableOptions.slice(
    ...
    public onUserSelectedOption(option: Option<any>, event) {
        ...
        const index: number = this.options.findIndex((eachOpt) =>
            this.valuesEqual(eachOpt.value, option.value),
        );
        this.onUserSelectedOptionIndex(index);
    }

    public onUserSelectedOptionIndex(index: number) {
        const promise: Promise<boolean> = this.validateChange
            ? this.validateChange(this.selectedOption?.value)
        ...
        promise.then((valid) => {
            if (valid) {
                ...
                this.onChangeCallback(
                    this.selectedOption ? this.selectedOption.value : null,
                );
    ...
    public isOptionSelected(option: Option<any>) {
        return (
            ...
            this.valuesEqual(this.selectedOption.value, option.value)
        );
    }
    ...
    private subscribeToGlobalClick() {
        this.globalClickSubscription =
            this.globalService.onGlobalClick.subscribe((clickEvent: Event) => {
    ...
    private unsubscribeToGlobalClick() {
        if (this.globalClickSubscription)
    ...
    onWheel(event) {
        if (
            event.wheelDelta < 0 &&
            this.firstScrollOptionIndex < this.length - this.LIST_LENGTH
        ) {
            this.firstScrollOptionIndex++;
        }
        ...
        if ("ArrowDown" == event.key) {
            ...
            if (!this.isOpen() && !this.disabled) {
                ...
                if (this.focusedOptionIndex != null) {
                    this.onUserSelectedOption(
                        this.displayedOptions[this.focusedOptionIndex],
                        event,
                    );
                }
            }
    ...
    private scrollDownByOne() {
        if (this.isOpen()) {
            const nextIndexStart: number =
                this.focusedOptionIndex != null &&
                this.focusedOptionIndex != undefined
                    ? this.focusedOptionIndex + this.firstScrollOptionIndex + 1
                    : 0;
            const nextIndex: number = this.displayableOptions
                .slice(nextIndexStart)
                .findIndex((opt) => {
                    return !opt.disabled;
                });
            if (nextIndex == -1) return;
            const nbSteps: number =
                this.focusedOptionIndex != null &&
                this.focusedOptionIndex != undefined
                    ? nextIndex + 1
                    : 0;
            if (
                this.scrollable &&
                this.focusedOptionIndex + nbSteps >= this.LIST_LENGTH
            ) {
        ...
        if (this.isOpen()) {
            const nextIndexStart: number =
                ...
                this.focusedOptionIndex != undefined
                    ? this.focusedOptionIndex + this.firstScrollOptionIndex - 1
                    : 0;
            const nextIndex: number = findLastIndex(
                this.displayableOptions.slice(0, nextIndexStart + 1),
                (opt) => {
                    return !opt.disabled;
                },
            );
    ...
    allowDrop(event) {
        ...
        if (dropY == this.lastDropY || Date.now() - this.lastScroll < 30)
            return;
        ...
        const relativeDropY = Math.max(
            0,
            dropY - listTop - this.dragStartOffsetY,
        );

        this.firstScrollOptionIndex = Math.min(
            Math.round(
                (this.displayableOptions.length - this.LIST_LENGTH) *
                    (relativeDropY / (listHeight - 50)),
            ),
            this.displayableOptions.length - this.LIST_LENGTH,
        );
    }
    ...
    onScrollZoneClick(event) {
        if (event.srcElement.classList.contains("scrollzone")) {
            if (
                event.clientY <
                this.liftHeight + event.srcElement.getBoundingClientRect().top
            ) {
                this.pageUp();
            } else {
                this.pageDown();
    ...
    private pageDown() {
        this.focusedOptionIndex = null;
        if (
            this.firstScrollOptionIndex + 2 * this.LIST_LENGTH <=
            this.displayableOptions.length
        ) {
            this.firstScrollOptionIndex += this.LIST_LENGTH;
        } else {
            this.firstScrollOptionIndex =
    ...
    clickView(): void {
        if (!this.viewDisabled && this.selectedOption)
            this.viewClick.emit(this.selectedOption.value);
    }
    ...
    clickAdd(): void {
        if (
            ...
            this.addClick.emit(this.selectedOption.value);
    }
    ...
    public get liftHeight(): number {
        return this.length > 0
            ? (250 * this.firstScrollOptionIndex) /
                  (this.length - this.LIST_LENGTH)
            : 0;
    }
...
export class Option<T> {
    ...
    clone(): Option<T> {
        const option: Option<T> = new Option(
            this.value,
            this.label,
            this.section,
        );
...
export class FilteredOptions {
    ...
    constructor(
        ...
        this._filteredList = options.filter((option) =>
            option.label.toLowerCase().includes(_searchText),
        );
    }
