...
export class KeycloakSessionService {
    ...
    constructor() {
        if (this.isAuthenticated()) {
            this.initSession(
                KeycloakService.auth.authz.sessionId,
                KeycloakService.auth.authz.refreshTokenParsed.exp,
            );
        }
        this.cleanStorage();
    }
    ...
    private initSession(sessionId: string, expirationTS: number) {
        ...
        if (!localStorage.getItem("kcsession-" + this._sessionId)) {
            this.saveSessionObject(
                new KeycloakSession(sessionId, expirationTS),
            );
        }
    }
    ...
    cleanStorage() {
        Object.keys(localStorage)
            .filter((key) => key.startsWith("kcsession-"))
            .forEach((key) => {
                const kcSession: KeycloakSession = JSON.parse(
