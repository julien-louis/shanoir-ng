    ...
    implements ControlValueAccessor, OnChanges
{
        ...
        if (
            ...
            this.optionArr.forEach((item) => {
                ...
                if (item.backgroundColor)
                    newOption.backgroundColor = item.backgroundColor;
            ...
            this.columnDefs?.push({
                ...
                action: (item) => this.onRemoveItem(item),
            });
        }
    }
    ...
    updateData() {
        ...
        this.modelArray.forEach((modelValue) => {
            const selected: Option<any> = this.options.find((option) =>
                objectsEqual(option.value, modelValue),
            );
            if (selected) {
    ...
    onSelectOption(id: number) {
        const option: Option<{ id: number }> = this.options?.find(
            (opt) => opt?.value?.id == id,
        );
    ...
    onRemoveItem(item: any) {
        ...
        const removed: Option<any> = this.options?.find(
            (option) => option.value?.id == item.id,
        );
