...
const USE_LOGIN_REQUIRED =
    (window as any).SHANOIR_KEYCLOAK_ADAPTER_MODE == "login-required";
...
export class KeycloakService {
            ...
            keycloakAuth
                ...
                .then((authenticated) => {
                    if (authenticated) {
                        ...
                        // Connected user id
                        KeycloakService.auth.userId =
                            keycloakAuth.tokenParsed.userId;
                        KeycloakService.auth.logoutUrl =
                            keycloakAuth.authServerUrl +
                            "/realms/shanoir-ng/protocol/openid-connect/logout?redirect_uri=" +
    ...
    logout() {
        if (KeycloakService.auth && KeycloakService.auth.authz)
            KeycloakService.auth.authz.logout({
                redirectUri: AppUtils.LOGOUT_REDIRECT_URL,
            });
    }
    ...
    isUserAdmin(): boolean {
        return (
            KeycloakService.auth.authz &&
            KeycloakService.auth.authz.hasRealmRole("ROLE_ADMIN")
        );
    }
    ...
    isUserExpert(): boolean {
        return (
            KeycloakService.auth.authz &&
            KeycloakService.auth.authz.hasRealmRole("ROLE_EXPERT")
        );
    }
    ...
    canUserImportFromPACS(): boolean {
        return (
            this.isUserAdmin() ||
            (KeycloakService.auth.authz &&
                KeycloakService.auth.authz.tokenParsed.canImportFromPACS)
        );
    }
