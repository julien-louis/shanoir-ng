...
export class DatasetCopyDialogComponent {
    ...
    private sortStudies() {
        ...
        for (const line of this.inputDatasets) {
            ...
            if (line.subjectId != null) {
                ...
                this.statusMessage =
                    "Some of the selected datasets (id = " +
                    ids.join(", ") +
                    ") have no subject, can't proceed with the copy.";
                this.canCopy = false;
            }
        }
    }
    ...
    public copy() {
        ...
        this.checkRightsOnSelectedStudies(this.selectedStudy.id).then(() => {
            this.isDatasetInStudy = this.checkDatasetBelongToStudy(
                this.inputDatasets,
            ...
            if (!this.hasRight) {
                this.statusMessage =
                    "Missing rights for study " +
                    this.selectedStudy.name +
                    " please make sure you have ADMIN right.";
            } else if (this.isDatasetInStudy) {
                this.statusMessage =
                    "Selected dataset(s) already belong to selected study.";
            } else {
                const formData: FormData = new FormData();
                formData.set(
                    "datasetIds",
                    Array.from(this.inputDatasets.map((d) => d.datasetId)).join(
                        ",",
                    ),
                );
                ...
                return firstValueFrom(
                    ...
                    .then(() => {
                        ...
                        this.consoleService.log(
                            "info",
                            "The copy of " +
                                this.inputDatasets.length +
                                " datasets towards study " +
                                this.selectedStudy.name +
                                " has started.",
                        );
                    })
