...
export abstract class EntityService<T extends Entity> implements OnDestroy {
    ...
    protected confirmDialogService =
    ...
    deleteWithConfirmDialog(
        ...
        return this.confirmDialogService
            .confirm(dialogTitle, customMsg || defaultMsg)
            .then((res) => {
                if (res) {
                    return this.delete(entity.id)
                        ...
                        .catch((reason) => {
                            ...
                            if (
                                (reason.error && reason.error.code == 422) ||
                                reason.status == 422
                            ) {
                            ...
                            if (
                                reason instanceof ShanoirError &&
                                reason.code == 422
                            ) {
                                if (reason.message) {
    ...
    protected toRealObject(entity: any): T {
        const trueObject = Object.assign(
            this.getEntityInstance(entity),
            entity,
        );
    ...
    protected customReplacer(key, value) {
        if (EntityService.getIgnoreList().indexOf(key) > -1) return undefined;
            ...
            return EntityService.datePattern(this[key]);
        else return value;
    }
