...
export class CheckboxListComponent implements ControlValueAccessor, OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes.items && this.items !== undefined) {
            this.totalPages = Math.ceil(
                this.items.totalElements / this.itemsPersPage,
            );
    ...
    search() {
        ...
        this.items.content = this.items.content.sort((a, b) => {
            if (
                a.value.indexOf(this.searchBarContent) == 0 &&
                b.value.indexOf(this.searchBarContent) != 0
            ) {
                return -1;
            } else if (
                a.value.indexOf(this.searchBarContent) != 0 &&
    ...
    onToggle() {
        this.selectedItems = this.items.content
