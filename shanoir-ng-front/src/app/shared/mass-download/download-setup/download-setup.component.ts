...
export class DownloadSetupComponent implements OnInit, OnDestroy {
    ...
    private buildForm(): UntypedFormGroup {
        ...
        this.subscriptions.push(
            formGroup.get("unzip").valueChanges.subscribe((val) => {
    ...
    downloadNow() {
        const setup: DownloadSetup = new DownloadSetup(
            this.form.get("format").value,
        );
        ...
        setup.converter =
            this.form.get("format").value == "nii"
                ...
                : null;
        if (this.form.get("shortPath"))
            setup.shortPath = this.form.get("shortPath").value;
    ...
    private hasDicomInDatasets(
        datasets: { type: DatasetType; hasProcessings: boolean }[],
    ) {
