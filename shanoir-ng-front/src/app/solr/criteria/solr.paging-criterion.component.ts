...
@Component({
    ...
    styleUrls: [
        "solr.criterion.component.css",
        "solr.paging-criterion.component.css",
    ],
    ...
    implements ControlValueAccessor, OnChanges
{
    ...
    loadedPromise: Promise<void> = new Promise(
        (resolve) => (this.loadedPromiseResolve = resolve),
    );
    ...
    @Output() sortModeChange: EventEmitter<"INDEX" | "COUNT"> =
        new EventEmitter();
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes.facetName) {
            this.hash = SolrPagingCriterionComponent.getHash(
                this.facetName,
                this.router.url,
            );
            this.reloadSettings();
    ...
    goToPage(pageNumber: number): Promise<void> {
        const pageable: FacetPageable = new FacetPageable(
            pageNumber,
        ...
        this.loading = true;
        return this.getPage(pageable, this.facetName)
            .then((page) => {
                this.loadPage(page);
                this.loaded = true;
    ...
    getCurrentPageable(pageNumber?: number): FacetPageable {
        return new FacetPageable(
            pageNumber ? pageNumber : this.currentPage?.number,
            SolrPagingCriterionComponent.PAGE_SIZE,
            this.sortMode,
            this.filterText,
        );
    ...
    onCheckChange(facet: FacetField) {
        if (facet.checked) {
            ...
            this.selectedFacets = this.selectedFacets.filter(
                (sel) => sel.value != facet.value,
            );
        }
        this.updateHasChecked();
        const selectedValues: string[] = this.selectedFacets.map(
            (facet) => facet.value,
        );
    ...
    writeValue(selectedFacetValues: string[]): void {
        this.loadedPromise.then(() => {
            ...
            selectedFacetValues?.forEach((val) => {
                const displayed: FacetField = this.displayedFacets.find(
                    (fac) => fac.value == val,
                );
                if (displayed) {
    ...
    reloadSettings() {
        ...
        if (prefStr) {
            ...
            if (pref.sortMode && pref.sortMode != this.sortMode)
                this.sortMode = pref.sortMode;
        }
    }
