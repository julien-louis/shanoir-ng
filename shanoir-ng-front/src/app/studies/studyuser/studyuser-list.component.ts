...
export class StudyUserListComponent implements ControlValueAccessor, OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes.studies && this.studies) {
            this.studyOptions = this.studies.map((study) => {
                const option: Option<Study> = new Option<Study>(
                    study,
                    study.name,
                );
                option.disabled = !!this.studyUserList?.find(
                    (su) => su.studyId == study.id || su.study?.id == study.id,
                );
                return option;
            });
        }
        ...
        if (changes.studyCenters) {
            ...
            this.studyUserList?.forEach((su) => {
                su.centers = su.centers?.filter(
                    (suc) => this.centers.findIndex((c) => c.id == suc.id) > -1,
                );
            });
        }
    }
    ...
    writeValue(obj: any): void {
        ...
        this.getPage = (pageable) =>
            Promise.resolve(this.browserPaging.getPage(pageable));
    }
    ...
    disableEdit(studyUser: StudyUser): boolean {
        return (
            !this.freshlyAddedMe &&
            studyUser.userId == KeycloakService.auth.userId
        );
    }
        ...
        this.columnDefs = [
            ...
            //     cellRenderer: (params: any) => !params.data.centers || params.data.centers.length == 0},
            {
                ...
                cellRenderer: (params: any) =>
                    params.data.studyUserRights.includes(
                        StudyUserRight.CAN_SEE_ALL,
                    ),
            },
            {
                headerName: "Can download",
                type: "boolean",
                editable: (su: StudyUser) => !this.disableEdit(su),
                width: "54px",
                disableSorting: true,
                onEdit: (su: StudyUser, value: boolean) =>
                    this.onEditRight(StudyUserRight.CAN_DOWNLOAD, su, value),
                cellRenderer: (params: any) =>
                    params.data.studyUserRights.includes(
            ...
            {
                headerName: "Can import",
                type: "boolean",
                editable: (su: StudyUser) => !this.disableEdit(su),
                width: "54px",
                disableSorting: true,
                onEdit: (su: StudyUser, value: boolean) =>
                    this.onEditRight(StudyUserRight.CAN_IMPORT, su, value),
                cellRenderer: (params: any) =>
                    params.data.studyUserRights.includes(
            ...
            },
            {
                ...
                disableSorting: true,
                onEdit: (su: StudyUser, value: boolean) =>
                    this.onEditRight(StudyUserRight.CAN_EXECUTE, su, value),
                cellRenderer: (params: any) =>
                    params.data.studyUserRights.includes(
                        StudyUserRight.CAN_EXECUTE,
                    ),
            },
            {
                headerName: "Can admin",
                type: "boolean",
                disableSorting: true,
                editable: (su: StudyUser) =>
                    su.user &&
                    su.user.role.displayName != "User" &&
                    !this.disableEdit(su),
                width: "54px",
                onEdit: (su: StudyUser, value: boolean) =>
                    this.onEditRight(
    ...
    onStudyAdd(selectedStudy: Study) {
        ...
        if (
            !this.studyUserList.find((su) => su.study?.id == selectedStudy.id)
        ) {
            return;
        }
    ...
    addStudy(
        ...
        if (this.studyOptions) {
            const option = this.studyOptions.find(
                (opt) => opt.value.id == selectedStudy.id,
            );
            if (option) option.disabled = true;
        }

        const backedUpStudyUser: StudyUser = this.studyUserBackup.find(
            (su) => su.study?.id == selectedStudy.id,
        );
    ...
    public inviteUser() {
        ...
        this.accessRequestService
            .inviteUser(this.invitationMail, this.invitationFunction, study)
            .then((request) => {
                if (!request) {
                    this.consoleService.log(
                        "info",
                        "No user found with such email, an invitation was sent.",
                    );
                } else {
            ...
            .catch(() => {
                this.consoleService.log(
                    "error",
                    "No user found with such login, please check the user information or use its email.",
                );
            });
    }
    ...
    public addUser(
        ...
        if (
            this.studyUserList.filter((user) => user.userId == selectedUser.id)
                .length > 0
        ) {
        ...
        const backedUpStudyUser: StudyUser = this.studyUserBackup.filter(
            (su) => su.userId == selectedUser.id,
        )[0];
        if (backedUpStudyUser) {
            this.studyUserList.unshift(backedUpStudyUser);
