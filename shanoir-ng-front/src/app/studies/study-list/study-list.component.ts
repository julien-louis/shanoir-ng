...
export class StudyListComponent extends BrowserPaginEntityListComponent<Study> {
    ...
    getEntities(eager: boolean = false): Promise<Study[]> {
        const earlyResult: Promise<Study[]> = Promise.all([
            this.studyService
                .getAll()
                .then((studies) => this.fetchStorageVolumesByChunk(studies)),
            ...
            studies = studies.concat(
                publicStudies
                    .filter(
                        (publicStudy) =>
                            !studies.find((s) => s.id == publicStudy.id),
                    )
                    .map((publicStudy) => {
                        const study: Study = new Study();
                        study.id = publicStudy.id;
                        study.downloadableByDefault =
                            publicStudy.downloadableByDefault;
            ...
            if (accessRequests?.length > 0) {
                for (const accessRequest of accessRequests) {
                    if (accessRequest.status == 0) {
                        studies.find(
                            (study) => study.id == accessRequest.studyId,
                        ).accessRequestedByCurrentUser = true;
                    }
                }
            }
            return studies;
    ...
    private fetchStorageVolumes(studies: Study[]): Promise<void> {
        return this.studyService
            .getStudiesStorageVolume(studies?.map((s) => s.id))
            .then((volumes) => {
        ...
        const colDef: ColumnDefinition[] = [
            {
                headerName: "",
                type: "boolean",
                awesome: "fa-solid fa-lock",
                awesomeFalse: "fa-solid fa-lock-open",
                color: "var(--color-a)",
                colorFalse: "var(--color-a)",
                cellRenderer: (ret) => {
                    return (
                        (ret.data as Study).visibleByDefault &&
                        (ret.data as Study).locked
                    );
                },
            },
            {
                ...
                type: "boolean",
                field: "isDraft",
                awesome: "fa-regular fa-clipboard",
                color: "var(--color-a)",
                width: "15px",
            ...
            { headerName: "Name", field: "name" },
            {
                ...
                cellRenderer: function (params: any) {
                    return capitalsAndUnderscoresToDisplayable(
                        params.data.studyStatus,
                    );
                },
            },
            ...
            {
                headerName: "Examinations",
                field: "nbExaminations",
                type: "number",
                width: "30px",
            },
            {
            ...
            },
            {
                ...
                cellRenderer: (params: any) => {
                    if (params.data?.totalSize >= 0) {
                        return this.studyService.storageVolumePrettyPrint(
                ...
                tip: (data: any) => {
                    ...
                    if (data.detailedSizes) {
                        data.detailedSizes.forEach(
                            (size: number, label: string) => {
                                tip +=
                                    label +
                                    " : " +
                                    this.studyService.storageVolumePrettyPrint(
                                        size,
                                    ) +
                                    "\n";
                            },
    ...
    canEdit(study: Study): boolean {
        return (
            ...
            this.studyIdsForCurrentUser.includes(study.id)
        );
    }

    private fetchStudyUsers(study: any): Promise<StudyUser[]> {
        return this.studyService
            .getStudyUserFromStudyId(study.id)
            .then((studyUsers) => {
                return studyUsers;
            });
    }
    ...
    goToViewFromEntity(study: any): void {
        Promise.all([
            ...
            if (
                study.visibleByDefault &&
                study.locked &&
                !this.keycloakService.isUserAdmin()
            ) {
                if (study.accessRequestedByCurrentUser) {
                    this.confirmDialogService.inform(
                        "Access request pending",
                        "You already have asked an access request for this study, wait for the administrator to confirm your access.",
                    );
                        ...
                        .then((result) => {
                            if (result) {
                                this.router.navigate([
                                    "/access-request/study/" + study.id,
                                ]);
                            }
                        });
                } else if (hasDUA && !this.isSuConfirmed) {
                    const title: string =
                        "Data User Agreement awaiting for signing";
                    const text: string =
