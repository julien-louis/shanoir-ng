...
export class StudyHistoryComponent implements OnInit {
    ...
    historyColumns: ColumnDefinition[] = [
        ...
        { headerName: "User", field: "username" },
        {
            ...
            cellRenderer: function (params: any) {
                if (params.data.eventType.includes(".event")) {
                    params.data.eventType = params.data.eventType.replace(
                        ".event",
                        "",
                    );
                }
        ...
        },
        {
            headerName: "ObjectId",
            field: "objectId",
            route: function (params: ShanoirEvent) {
                const event = params.eventType;
                const id = params.objectId;
                if (event.includes("create")) {
                    if (
                        event.includes("Dataset") &&
                        !event.includes("Acquisition")
                    ) {
                        return "/dataset/details/" + id;
                    } else if (event.includes("Examination")) {
                        return "/examination/details/" + id;
                    } else if (event.includes("Acquisition")) {
                        return "/dataset-acquisition/details/" + id;
                    } else if (event.includes("Subject")) {
                        return "/subject/details/" + id;
                    }
                } else if (event.includes("update")) {
                    if (event.includes("Study")) {
                        return "/study/details/" + id;
                    } else if (event.includes("Examination")) {
                        return "/examination/details/" + id;
                    } else if (event.includes("Subject")) {
                        return "/subject/details/" + id;
    ...
    constructor(private shanoirEventService: ShanoirEventService) {}
    ...
    getPage(pageable: Pageable): Promise<Page<ShanoirEvent> | void> {
        return this.shanoirEventService
            .getPage(
                pageable,
                this.study.id,
