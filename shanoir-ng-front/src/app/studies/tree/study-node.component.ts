...
export class StudyNodeComponent
    extends TreeNodeAbstractComponent<StudyNode>
    implements OnChanges
{
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes["input"]) {
            const id: number =
                this.input instanceof StudyNode
                    ? this.input.id
                    : this.input.study.id;
            ...
            if (this.input instanceof StudyNode) {
                ...
                this.node = this.treeService.buildStudyNode(
                    this.input.study,
                    this.input.rights,
                );
            } else {
            ...
            this.showDetails =
                this.router.url != this.detailsPath + this.node.id;
            setTimeout(() => {
                if (this.fakeSubjectNodes.length > 0) {
                    this.setFakeTops();
                }
                this.subscriptions.push(
                    this.fakeSubjectNodes.changes.subscribe(() =>
                        this.setFakeTops(),
                    ),
                );
            });
    ...
    private setFakeTops() {
        this.fakeSubjectNodes.forEach((fake) => {
            ...
            if (
                !!this.node.subjectsNode.subjects &&
                this.node.subjectsNode.subjects != UNLOADED
            ) {
                const node: SubjectNode = this.node.subjectsNode.subjects?.find(
                    (n) => n.id == id,
                );
        ...
        this.fakeMemberNodes.forEach((fake) => {
            const id: number = fake.nativeElement.getAttribute("id");
            if (
                !!this.node.membersNode.members &&
                this.node.membersNode.members != UNLOADED
            ) {
                const node: MemberNode = this.node.membersNode.members?.find(
                    (n) => n.id == id,
                );
    ...
    loadStudyCards() {
        if (this.node.studyCardsNode.cards == UNLOADED) {
            ...
            this.studyCardService
                .getAllForStudy(this.node.id)
                .then((studyCards) => {
                    if (studyCards) {
                        this.node.studyCardsNode.cards = studyCards.map(
                            (studyCard) =>
                                new StudyCardNode(
                                    this.node,
                                    studyCard.id,
    ...
    loadQualityCards() {
        if (this.node.qualityCardsNode.cards == UNLOADED) {
            ...
            this.qualityCardService
                ...
                .then((qualityCards) => {
                    if (qualityCards) {
                        this.node.qualityCardsNode.cards = qualityCards.map(
                            (studyCard) =>
                                new QualityCardNode(
                                    this.node,
                                    studyCard.id,
    ...
    onFilterChange() {
        if (
            ...
            this.filteredNodes = (
                this.node.subjectsNode.subjects as SubjectNode[]
            ).filter((node) => {
                return node.label
                    .toLowerCase()
    ...
    sortSubjects(sort: Sort) {
        ...
        if (!(this.node.subjectsNode.subjects == "UNLOADED")) {
            if (this.subjectsOrder.field == "name") {
                this.node.subjectsNode.subjects.sort((a, b) =>
    ...
    trackByFn(index, item: ShanoirNode) {
        return item.id;
    }
