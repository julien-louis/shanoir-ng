...
export class DicomUploadComponent implements OnDestroy {
    ...
    private uploadToServer(file: any) {
        ...
        if (!this.multipleExamImport) {
                ...
                this.importService.uploadFile(formData).subscribe(
                    (event) => {
                        if (event.type === HttpEventType.Sent) {
                            ...
                            this.importDataService.patientList =
                                patientDicomList;
            ...
            job.acquisitionEquipmentId = this.useStudyCard
                ? this.studyCard.acquisitionEquipment.id
                : this.acquisitionEquipment.id;
            job.studyId = this.study.id;
            job.studyName = this.study.name;
            job.studyCardId = this.useStudyCard ? this.studyCard?.id : 0;
            job.centerId = this.useStudyCard
                ? this.studyCard.acquisitionEquipment.center.id
                : this.acquisitionEquipment.center.id;
            job.anonymisationProfileToUse = this.study.profile.profileName;

            this.subscriptions.push(
                this.importService.uploadFileMultiple(formData, job).subscribe(
                    (event) => {
                        if (event.type === HttpEventType.Sent) {
                            this.uploadState.progress = -1;
                        } else if (
