...
export const atLeastOneNotBlank =
    ...
    (group: UntypedFormGroup): ValidationErrors | null => {
        const hasAtLeastOneNotBlank =
            group &&
            group.controls &&
            Object.keys(group.controls).some(
                (key) =>
                    !validator(group.controls[key]) &&
                    group.controls[key].value.trim().length != 0,
            );
...
export class QueryPacsComponent {
            ...
            .then((importJob: ImportJob) => {
                        ...
                        .map((patient) => ({
                            ...
                            studies: patient.studies
                                .map((study) => ({
                                    ...study,
                                    series:
                                        this.dicomQuery.modality == "" // if modality is 'none', do not filter
                                            ? study.series
                                            : study.series.filter(
                                                  (serie) =>
                                                      serie.modality ===
                                                      this.dicomQuery.modality,
                                              ),
                                }))
                ...
                if (importJob && importJob.patients.length > 0) {
                    ...
                    this.consoleService.log(
                        "warn",
                        "Nothing found. Please change your query parameters.",
                        ["query : " + JSON.stringify(this.dicomQuery)],
                    );
                }
            });
    }
    ...
    buildForm(): void {
        ...
        this.form = this.formBuilder.group(
            {
                patientName: [
                    this.dicomQuery.patientName,
                    [
                        Validators.maxLength(64),
                        Validators.pattern(noWildcardPattern),
                    ],
                ],
                patientID: [
                    this.dicomQuery.patientID,
                    [
                        Validators.maxLength(64),
                        Validators.pattern(noWildcardPattern),
                    ],
                ...
                patientBirthDate: [
                    this.dicomQuery.patientBirthDate,
                    Validators.pattern(pacsDatePattern),
                ],
                studyDescription: [
                    this.dicomQuery.studyDescription,
                    [Validators.maxLength(64), Validators.minLength(4)],
                ],
                studyDate: [
                    this.dicomQuery.studyDate,
                    Validators.pattern(pacsDatePattern),
                ],
