...
export class DUASigningComponent implements OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes["dua"]) {
            if (this.dua && this.dua.studyId) {
                this.studyService
                    .downloadDuaBlob(this.dua.path, this.dua.studyId)
                    .then((response) => {
                        this.duaBlob = response;
                        const url: SafeResourceUrl =
                            this.sanitizer.bypassSecurityTrustResourceUrl(
                                URL.createObjectURL(this.duaBlob),
                            );
    ...
    refuse() {
        const confirmMsg: string =
            "Do you really want to refuse the Data User Agreement for the study xxxx ? You will be removed from this study and won't be asked again.";
