...
export class CenterListComponent extends BrowserPaginEntityListComponent<Center> {
    ...
    getColumnDefs(): ColumnDefinition[] {
        ...
        if (this.keycloakService.isUserAdminOrExpert()) {
            columnDefs.push({
                headerName: "",
                type: "button",
                awesome: "fa-solid fa-microscope",
                tip: () => {
                    return "Add acq. equip.";
                },
                action: (item) => this.openCreateAcqEquip(item),
            });
        }
        return columnDefs;
    }
        ...
        this.router.navigate(["/acquisition-equipment/create"]).then(() => {
            this.subscriptions.push(
                currentStep
                    ...
                    .subscribe((entity) => {
                        center.acquisitionEquipments.push(
    ...
    private manageDelete() {
        this.subscriptions.push(
            this.onDelete.subscribe((response) => {
                if (
                    response.error &&
                    response.error instanceof ShanoirError &&
                    response.error.code == 422
                ) {
                    const msg: string = this.buildDeleteErrMsg(
                        response as { entity: Center; error: ShanoirError },
                    );
    ...
    private buildDeleteErrMsg(response: {
        entity: Center;
        error: ShanoirError;
    }): string {
        ...
        let msg: string =
            ...
            '" cannot be deleted. It is associated with ';
