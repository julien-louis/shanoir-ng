...
export namespace CardinalityOfRelatedSubjects {
    export function all(): CardinalityOfRelatedSubjects[] {
        return allOfEnum<CardinalityOfRelatedSubjects>(
    ...
    export const options: Option<CardinalityOfRelatedSubjects>[] = all().map(
        (prop) =>
            new Option<CardinalityOfRelatedSubjects>(prop, getLabel(prop)),
    );
}
