...
export class AccessRequestComponent extends EntityComponent<AccessRequest> {
    ...
    public get accessRequest(): AccessRequest {
        return this.entity;
    }
    ...
    public changeStudy(studyId: number) {
        this.accessRequest.studyName =
            this.studies[
                this.studies.findIndex((study) => study.id == studyId)
            ].name;
    }
        ...
        return this.studyService.getPublicStudiesConnected().then((result) => {
            if (result && result.length > 0) {
                ...
                this.confirmDialogService
                    .error(
                        "No public study",
                        "No public studies available for the moment. If you want to join a private study, please ask the study manager to add you directly.",
                    )
                    .then(() => this.goBack());
            }
        });
    ...
    checkAccess(studyId: number) {
        this.studyService.getStudiesNames().then((studies) => {
            if (studies?.find((s) => s.id == studyId)) {
                this.confirmDialogService
                    .inform(
                        "You already have access to this study",
                        "No need to request an access.",
                        "Go to the study",
                    )
                    .then(() => {
    ...
    acceptRequest() {
        this.accessRequestService
            .resolveRequest(this.accessRequest.id, true)
            .then(() => {
                this.userService.decreaseAccessRequests();
                this.router.navigate([
                    "/study/details/" + this.accessRequest.studyId,
                ]);
            })
            .then(() => {
    ...
    refuseRequest() {
        this.accessRequestService
            .resolveRequest(this.accessRequest.id, false)
            .then(() => {
                this.userService.decreaseAccessRequests();
                this.goBack();
            });
