...
export class UserListComponent extends BrowserPaginEntityListComponent<User> {
    ...
    getEntities(eager: boolean = false): Promise<User[]> {
        ...
        // get the study-users
        const allPromise: Promise<User[]> = Promise.all([
            userPromise,
            ...
            users.forEach((user) => {
                ...
                studies.forEach((study) =>
                    user.studyUserList.push(
                        ...study.studyUserList
                            .filter(
                                (studyUser) =>
                                    (studyUser.user
                                        ? studyUser.user.id
                                        : studyUser.userId) == user.id,
                            )
                            .map((studyUser) => {
    ...
    getColumnDefs(): ColumnDefinition[] {
        const columnDefs: ColumnDefinition[] = [
            ...
            { headerName: "Email", field: "email", width: "200%" },
            {
                ...
                cellRenderer: function (params: any) {
                    if (params.data.studyUserList) {
                        return (params.data.studyUserList as StudyUser[])
                            .map((su) => su.study?.name)
                            .join(" - ");
                    } else {
                        return "";
                    }
            ...
            },
            {
                ...
                cellRenderer: function (params: any) {
                    return (
                        params.data.accountRequestDemand ||
                        params.data.extensionRequestDemand
                    );
                },
            },
            {
                headerName: "Challenge",
                field: "accountRequestInfo.challenge",
                type: "boolean",
                defaultSortCol: true,
            },
            ...
            { headerName: "Expiration", field: "expirationDate", type: "date" },
            {
                ...
                cellRenderer: function (params: any) {
                    return (
                        ...
                        params.data.expirationDate >= new Date()
                    );
                },
            },
