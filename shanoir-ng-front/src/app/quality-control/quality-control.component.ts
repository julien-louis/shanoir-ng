...
export class QualityControlComponent implements OnChanges {
    ...
    getPage: Map<number, (FilterablePageable) => Promise<Page<any>>> =
        new Map();
        ...
        this.confirmService
            ...
            .then((accept) => {
                if (accept) {
                    ...
                    this.qualityCardService
                        .applyOnStudy(qualityCard.id)
                        .then((result) => {
                            this.pagings.set(
                                qualityCard.id,
                                new BrowserPaging(result, this.columnDefs),
                            );
                            this.getPage.set(
                                qualityCard.id,
    ...
    downloadReport(qualityCard: QualityCard) {
        const browserPaging: BrowserPaging<any> = this.pagings.get(
            qualityCard?.id,
        );
        let csvStr: string = "";
        csvStr += browserPaging.columnDefs
            .map((col) => col.headerName)
            .join(",");
        for (const entry of browserPaging.items) {
            csvStr +=
                "\n" +
                browserPaging.columnDefs
                    .map(
                        (col) =>
        ...
        AppUtils.browserDownloadFile(
            csvBlob,
            this.getReportFileName(qualityCard),
        );
    }
