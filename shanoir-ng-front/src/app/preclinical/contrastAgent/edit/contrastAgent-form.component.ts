...
export class ContrastAgentFormComponent extends EntityComponent<ContrastAgent> {
    ...
    get agent(): ContrastAgent {
        return this.entity;
    }
    set agent(agent: ContrastAgent) {
        this.entity = agent;
    }
    ...
    initView(): Promise<void> {
        ...
        this.loadReferences();
        return this.contrastAgentsService
            .getContrastAgent(this.protocol_id)
            .then((agent) => {
                if (agent) {
                    agent.name = this.getReferenceById(agent.name);
                    agent.doseUnit = this.getReferenceById(agent.doseUnit);
                    agent.concentrationUnit = this.getReferenceById(
                        agent.concentrationUnit,
                    );
                    this.agent = agent;
                }
            });
    ...
    loadReferences() {
        this.referenceService
            .getReferencesByCategoryAndType(
                PreclinicalUtils.PRECLINICAL_CAT_CONTRAST_AGENT,
                PreclinicalUtils.PRECLINICAL_CONTRAST_AGENT_NAME,
            )
            .then((names) => {
        ...
        this.referenceService
            ...
            .then((units) => {
                ...
                this.references = this.agentNames.concat(
                    this.doseUnits.concat(this.concentration_units),
                );
            });
    }

    goToAddContrastAgent() {
        this.router.navigate(["/preclinical-reference/create"], {
            queryParams: { category: "contrastagent", reftype: "name" },
        });
    }
    ...
    addContrastAgent() {
        ...
        this.contrastAgentsService.createConstrastAgent(
            this.protocol_id,
            this.agent,
        );
    }
