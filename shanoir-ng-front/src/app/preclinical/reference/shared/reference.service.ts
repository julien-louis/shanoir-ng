...
export class ReferenceService extends EntityService<Reference> {
    ...
    getCategories(): Promise<string[]> {
        return firstValueFrom(
            this.http.get<string[]>(
                PreclinicalUtils.PRECLINICAL_API_REFERENCES_CATEGORIES_ALL_URL,
            ),
        )
            .then((response) => response)
            .catch((error) => {
                console.error(
                    "Error while getting references categories",
                    error,
                );
    ...
    getTypesByCategory(category: string): Promise<string[]> {
        return firstValueFrom(
            this.http.get<string[]>(
                PreclinicalUtils.PRECLINICAL_API_REFERENCES_URL +
                    "/category/" +
                    category +
                    "/types",
            ),
            ...
            .catch((error) => {
                console.error(
                    "Error while getting references types by category",
                    error,
                );
                return Promise.reject(error.message || error);
            });
    ...
    getReferenceByCategoryTypeAndValue(
        category: string,
        reftype: string,
        value: string,
