...
@Component({
    ...
    styleUrls: [
        "../../../subjects/subject/subject.component.css",
...
export class AnimalSubjectFormComponent extends EntityComponent<AnimalSubject> {
    ...
    public get animalSubject(): AnimalSubject {
        return this.entity;
    }
    public set animalSubject(animalSubject: AnimalSubject) {
        this.entity = animalSubject;
    }
    ...
    protected fetchEntity: () => Promise<AnimalSubject> = () => {
        return this.idPromise.then((id) => {
            return this.getService()
                .get(id)
                .then((animalSubject) => {
                    return this.subjectService.get(id).then((subject) => {
    ...
    initView(): Promise<void> {
        this.studyService
            .get(this.animalSubject.subject?.study?.id)
            .then((study) => {
        ...
        return this.loadData().then(() => {
            this.subjectTherapyService
                .getSubjectTherapies(this.animalSubject)
                .then((st) => {
                    this.animalSubject.subjectTherapies = st;
                });
            this.subjectPathologyService
                .getSubjectPathologies(this.animalSubject)
                .then((sp) => {
    ...
    initEdit(): Promise<void> {
        ...
        return this.loadData().then(() => {
            this.animalSubject.specie = this.getReferenceById(
                this.animalSubject.specie,
            );
            this.animalSubject.strain = this.getReferenceById(
                this.animalSubject.strain,
            );
            this.animalSubject.biotype = this.getReferenceById(
                this.animalSubject.biotype,
            );
            this.animalSubject.provider = this.getReferenceById(
                this.animalSubject.provider,
            );
            this.animalSubject.stabulation = this.getReferenceById(
                this.animalSubject.stabulation,
            );
        });
    }
    ...
    initCreate(): Promise<void> {
        return this.loadData().then(() => {
            ...
            this.animalSubject.subject.imagedObjectCategory =
                ImagedObjectCategory.LIVING_ANIMAL;
        });
    }
        ...
        return Promise.all([
            this.referenceService
                .getReferencesByCategory(
                    PreclinicalUtils.PRECLINICAL_CAT_SUBJECT,
                )
                .then((references) => {
    ...
    public animalSelected(): boolean {
        return (
            this.animalSubject &&
            this.animalSubject.subject &&
            this.animalSubject.subject.imagedObjectCategory != null &&
            this.animalSubject.subject.imagedObjectCategory.toString() !=
                "PHANTOM" &&
            this.animalSubject.subject.imagedObjectCategory.toString() !=
                "ANATOMICAL_PIECE"
        );
    ...
    buildForm(): UntypedFormGroup {
        ...
        const subjectGroup = this.formBuilder.group({
            ...
            isAlreadyAnonymized: [
                this.animalSubject.subject?.isAlreadyAnonymized,
            ],
            name: [
                this.animalSubject.subject?.name,
                this.nameValidators.concat([
                    this.registerOnSubmitValidator("unique", "subject.name"),
                ]),
            ],
            sex: [
                this.animalSubject.subject?.sex,
        ...
        setTimeout(() => {
            if (
                this.breadcrumbsService.currentStep.isPrefilled(
                    "entity.subjectTherapies",
                )
            ) {
                animalSubjectGroup.get("subjectTherapies").markAsDirty();
                animalSubjectGroup
                    .get("subjectTherapies")
                    .updateValueAndValidity();
            }
            if (
                this.breadcrumbsService.currentStep.isPrefilled(
                    "entity.subjectPathologies",
                )
            ) {
                animalSubjectGroup.get("subjectPathologies").markAsDirty();
                animalSubjectGroup
                    .get("subjectPathologies")
                    .updateValueAndValidity();
            }
        });
    ...
    protected openCreateSubjectTherapy() {
        const currentTherapies: SubjectTherapy[] =
            this.animalSubject.subjectTherapies || [];
        const currentStep: Step = this.breadcrumbsService.currentStep;
        this.router
            .navigate(["/preclinical-subject-therapy/create"])
            .then(() => {
    ...
    protected openCreateSubjectPathology() {
        const currentPathologies: SubjectPathology[] =
            this.animalSubject.subjectPathologies || [];
        const currentStep: Step = this.breadcrumbsService.currentStep;
        this.router
            .navigate(["/preclinical-subject-pathology/create"])
            .then(() => {
                return firstValueFrom(
                    currentStep.waitFor(this.breadcrumbsService.currentStep),
                ).then((pathology) => {
    ...
    protected prefillProperties(): void {
        ...
        this.breadcrumbsService.currentStep
            .getPrefilledValue("newSubjectPathology")
            .then((newSubjectPathology: SubjectPathology) => {
                this.breadcrumbsService.currentStep.removePrefilled(
                    "newSubjectPathology",
                );
                if (newSubjectPathology) {
                    if (!this.animalSubject.subjectPathologies) {
                        ...
                        this.animalSubject.subjectPathologies.push(
                            newSubjectPathology,
                        );
                        this.animalSubject.subjectPathologies =
                            this.animalSubject.subjectPathologies.slice(); // Force array update
                    }
        ...
        this.breadcrumbsService.currentStep
            .getPrefilledValue("newSubjectTherapy")
            .then((newSubjectTherapy: SubjectTherapy) => {
                this.breadcrumbsService.currentStep.removePrefilled(
                    "newSubjectTherapy",
                );
                if (newSubjectTherapy) {
                    if (!this.animalSubject.subjectTherapies) {
                        ...
                        this.animalSubject.subjectTherapies.push(
                            newSubjectTherapy,
                        );
                        this.animalSubject.subjectTherapies =
                            this.animalSubject.subjectTherapies.slice(); // Force array update
                    }
    ...
    onChangeImagedObjectCategory(formGroup: UntypedFormGroup) {
        ...
        const newCategory: ImagedObjectCategory = subjectGroup.get(
            "imagedObjectCategory",
        ).value;
        const animalSubjectRequiredProperties: string[] = [
            "specie",
            "strain",
            "biotype",
            "provider",
    ...
    goToRefPage(category: string, reftype: string): void {
        this.navigateToAttributeCreateStep(
            "/preclinical-reference/create",
            "animalSubject." + reftype,
            null,
    ...
    public save(): Promise<AnimalSubject> {
        if (this.animalSubject.id) {
            return this.updateSubject()
                .then((createdAnimal) => {
                    ...
                    this.consoleService.log(
                        "info",
                        "Preclinical subject n°" +
                            this.animalSubject.id +
            ...
            this.animalSubject.subject = {
                ...this.animalSubject.subject,
                study: { id: this.animalSubject.subject.study.id } as Study,
            };
            return this.addSubject()
                .then((subject) => {
                    ...
                    this.consoleService.log(
                        "info",
                        "New preclinical subject successfully saved with n° " +
                            this.animalSubject.id,
                    );
                    return subject;
                })
    ...
    addSubject(): Promise<AnimalSubject> {
        ...
        this.animalSubject.subject.identifier =
            this.generateSubjectIdentifier();
        return this.animalSubjectService
            .create(this.animalSubject)
            .then((animalSubject) => {
    ...
    updateSubject(): Promise<AnimalSubject> {
        ...
        return this.subjectService
            .update(this.animalSubject.id, this.animalSubject.subject)
            .then(() => {
                if (this.animalSubject) {
                    this.animalSubjectService
                        .updateAnimalSubject(this.animalSubject)
    ...
    deleteBySubject(subject: Subject) {
        this.subjectService
            .deleteWithConfirmDialog("preclinical-subject", subject)
            .then((deleted) => {
                if (deleted) {
                    this.goToList();
                }
