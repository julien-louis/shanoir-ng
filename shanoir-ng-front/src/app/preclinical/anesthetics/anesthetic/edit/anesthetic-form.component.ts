...
export class AnestheticFormComponent extends EntityComponent<Anesthetic> {
    ...
    get anesthetic(): Anesthetic {
        return this.entity;
    }
    set anesthetic(anesthetic: Anesthetic) {
        this.entity = anesthetic;
    }
    ...
    initView(): Promise<void> {
        ...
        if (this.anesthetic && this.anesthetic.id) {
            this.ingredientService
                .getIngredients(this.anesthetic)
                .then((ingredients) => {
                    if (ingredients) {
    ...
    buildForm(): UntypedFormGroup {
        return this.formBuilder.group({
            ...
            anestheticType: [
                this.anesthetic.anestheticType,
                Validators.required,
            ],
    ...
    private createColumnDefs() {
        ...
        if (this.mode != "view" && this.keycloakService.isUserAdminOrExpert()) {
            this.columnDefs.push({
                ...
                action: (item) => this.editIngredient(item),
            });
        }
        if (this.mode != "view" && this.keycloakService.isUserAdminOrExpert()) {
            this.columnDefs.push({
                ...
                action: (item) => this.removeIngredient(item),
            });
        }
    }
            ...
            this.onSave.subscribe((response) => {
                if (this.ingredientsToDelete) {
                    for (const ingredient of this.ingredientsToDelete) {
                        this.ingredientService.deleteAnestheticIngredient(
                            response.id,
                            ingredient.id,
                        );
                    }
                ...
                if (this.ingredientsToCreate) {
                    for (const ingredient of this.ingredientsToCreate) {
                        this.ingredientService
                            .createAnestheticIngredient(response.id, ingredient)
                            .subscribe();
                    }
                }
            }),
    ...
    refreshName(generatedName: string) {
        if (
            this.anesthetic &&
            this.anesthetic.anestheticType &&
            generatedName.indexOf(
                AnestheticType[this.anesthetic.anestheticType],
            ) < 0
        ) {
            generatedName = AnestheticType[this.anesthetic.anestheticType]
                .concat(" ")
                .concat(generatedName);
        }
