...
export class ExaminationAnestheticFormComponent extends EntityComponent<ExaminationAnesthetic> {
    ...
    get examinationAnesthetic(): ExaminationAnesthetic {
        return this.entity;
    }
    set examinationAnesthetic(examinationAnesthetic: ExaminationAnesthetic) {
        this.entity = examinationAnesthetic;
    }
    ...
    protected fetchEntity: () => Promise<ExaminationAnesthetic> = () => {
        return this.examAnestheticService
            .getExaminationAnesthetics(this.id)
            .then((examAnesthetics) => {
                if (examAnesthetics && examAnesthetics.length > 0) {
                    //Should be only one
                    const examAnesthetic: ExaminationAnesthetic =
                        examAnesthetics[0];
    ...
    initView(): Promise<void> {
        ...
        return this.loadAllData().then(() => {
            if (this.id) return this.loadExistingAnesthetic();
        });
    }
    ...
    initEdit(): Promise<void> {
        ...
        return this.loadAllData().then(() => {
            ...
            if (this.examinationAnesthetic.doseUnit)
                this.examinationAnesthetic.doseUnit = this.findReferenceById(
                    this.examinationAnesthetic.doseUnit,
                );

            if (this.examinationAnesthetic.anesthetic)
                this.examinationAnesthetic.anesthetic = this.findAnestheticById(
                    this.examinationAnesthetic.anesthetic,
                );
        });
    }
    ...
    buildForm(): UntypedFormGroup {
        ...
        this.subscriptions.push(
            form.valueChanges.subscribe(() => this.eaChange()),
        );
    ...
    private findAnestheticById(anesthetic: any): Anesthetic {
        ...
        const anestheticId =
            typeof anesthetic === "object" ? anesthetic.id : anesthetic;
        return (
            this.anesthetics.find((anest) => anest.id === anestheticId) || null
        );
    }
    ...
    public save(): Promise<ExaminationAnesthetic> {
        if (this.examinationAnesthetic.id) {
            ...
            return this.addExaminationAnesthetic().then((exam) => {
                ...
                this.consoleService.log(
                    "info",
                    "New preclinical examination successfully saved with n°" +
                        this.examinationAnesthetic.id,
                );
                return exam;
            });
    ...
    addExaminationAnesthetic(): Promise<ExaminationAnesthetic> {
        if (!this.examinationAnesthetic) {
            ...
            return Promise.resolve(
                this.examAnestheticService.createAnesthetic(
                    this.examinationAnesthetic.examinationId,
                    this.examinationAnesthetic,
                ),
            );
        }
    }
    ...
    updateExaminationAnesthetic(): Promise<ExaminationAnesthetic> {
        return this.examAnestheticService
            ...
            .then(() => this.examinationAnesthetic);
    }
