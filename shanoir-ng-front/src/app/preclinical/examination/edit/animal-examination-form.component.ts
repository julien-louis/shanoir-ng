...
export class AnimalExaminationFormComponent extends ExaminationComponent {
    ...
    initEdit(): Promise<void> {
        ...
        if (
            this.examination &&
            this.examination.subject &&
            this.examination.subject.id
        ) {
            this.animalSubjectService
                .getAnimalSubject(this.examination.subject.id)
                .then(
                    (animalSubject) =>
                        (this.animalSubjectId = animalSubject.id),
                );
        }
    ...
    manageExaminationAnesthetic(examinationId: number) {
        if (this.examAnesthetic) {
            ...
            if (this.examAnesthetic && this.examAnesthetic.internalId) {
                this.examAnestheticService.updateAnesthetic(
                    examinationId,
                    this.examAnesthetic,
                );
    ...
    addExtraDataToExamination(examinationId: number) {
        ...
        if (this.physioDataFile) {
            ...
            if (this.physioDataFile?.id) {
                this.extradatasService
                    .updateExtradata(
                        PreclinicalUtils.PRECLINICAL_PHYSIO_DATA,
                        this.physioDataFile.id,
                        this.physioDataFile,
                    )
                    .then(() => {
                        if (this.physioDataFile.physiologicalDataFile) {
                            this.extradatasService
                                .postFile(
                                    this.physioDataFile.physiologicalDataFile,
                                    this.physioDataFile,
                                )
                                .then(() => {
                                    this.examinationExtradatas.push(
                                        this.physioDataFile,
                                    );
                                });
                        }
        ...
        if (this.bloodGasDataFile) {
            ...
            if (this.bloodGasDataFile.id) {
                this.extradatasService
                    .updateExtradata(
                        PreclinicalUtils.PRECLINICAL_BLOODGAS_DATA,
                        this.bloodGasDataFile.id,
                        this.bloodGasDataFile,
                    )
                    .then(() => {
                        if (this.bloodGasDataFile.bloodGasDataFile) {
                            this.extradatasService
                                .postFile(
                                    this.bloodGasDataFile.bloodGasDataFile,
                                    this.bloodGasDataFile,
                                )
                                .then(() => {
                                    this.examinationExtradatas.push(
    ...
    onUploadPhysiologicalData(event) {
        ...
        const fileChanged: boolean =
            this.physioDataFile?.physiologicalDataFile?.name !=
            event?.physiologicalDataFile?.name;
