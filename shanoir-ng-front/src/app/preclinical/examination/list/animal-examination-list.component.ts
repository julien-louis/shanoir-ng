...
export class AnimalExaminationListComponent extends EntityListComponent<Examination> {
    ...
    constructor(
        ...
        this.subscriptions.push(
            this.onDelete.subscribe((response) =>
                this.deleteExaminationRelatedData(response.entity.id),
            ),
        );
    }
        ...
        const colDef: ColumnDefinition[] = [
            { headerName: "Id", field: "id" },
            {
                ...
                cellRenderer: (params: any) =>
                    params.data.subject ? params.data.subject.name : "",
                route: (examination: Examination) =>
                    examination.subject
                        ? "/preclinical-subject/details/" +
                          examination.subject.id
                        : null,
            },
            {
    ...
    async deleteExaminationRelatedData(examinationId: number): Promise<void> {
        ...
        const examAnesthetics =
            await this.examAnestheticsService.getExaminationAnesthetics(
                examinationId,
            );
        if (examAnesthetics?.length)
            await this.examAnestheticsService.deleteAnesthetic(
                examAnesthetics[0],
            );

        // Delete all extra data (0 or multiple)
        const extraData =
            await this.extradataService.getExtraDatas(examinationId);
        if (extraData?.length)
            await Promise.all(
                extraData.map((data) =>
                    this.extradataService.deleteExtradata(data),
                ),
            );
    }
}
