...
export class PathologyModelsListComponent extends BrowserPaginEntityListComponent<PathologyModel> {
    ...
    getColumnDefs(): ColumnDefinition[] {
        const colDef: ColumnDefinition[] = [
            ...
            {
                headerName: "Specifications file",
                field: "filename",
                type: "string",
            },
        ];
        if (this.keycloakService.isUserAdminOrExpert()) {
            colDef.push({
                ...
                action: (item) => this.downloadModelSpecifications(item),
            });
        }
        return colDef;
    }
    ...
    openInformationDialog = (model: PathologyModel) => {
        this.confirmDialogService.inform(
            ...
            "No specifications have been found for " + model.name,
        );
    };

    protected openDeleteConfirmDialog = (entity: PathologyModel) => {
        this.subjectPathologyService
            .getAllSubjectForPathologyModel(entity.id)
            .then((subjectPathologies) => {
                if (subjectPathologies) {
                    let hasSubjects: boolean = false;
                    hasSubjects = subjectPathologies.length > 0;
                    if (hasSubjects) {
                        this.confirmDialogService.confirm(
                            "Delete pathology model",
                            "This pathology model is linked to subjects, it can not be deleted",
                        );
                    } else {
    ...
    private openDeletePathologyModelConfirmDialog = (
        entity: PathologyModel,
    ) => {
        if (!this.keycloakService.isUserAdminOrExpert()) return;
        this.confirmDialogService
            .confirm(
                "Delete",
                "Are you sure you want to delete preclinical-pathology-model n° " +
                    entity.id +
                    " ?",
            )
            .then((res) => {
                    ...
                    this.getService()
                        ...
                        .then(() => {
                            ...
                            this.consoleService.log(
                                "info",
                                "The preclinical-pathology-model n° " +
                                    entity.id +
