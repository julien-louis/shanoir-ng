    ...
    implements AfterViewInit, OnDestroy, AfterViewChecked
{
    ...
    constructor(
        ...
        this.onViewCheck = new WaitBurstEnd(
            () => this.onViewChecked.next(),
            300,
        );
        this.checkWidthBurst = new WaitBurstEnd(
            this._checkWidth.bind(this),
            200,
        );

        this.subscriptions.push(
            service.onUpdateSteps.subscribe((ret) => {
                this.steps = ret.steps.filter((step) => !step.disabled);
                setTimeout(() => {
                    if (ret.operation == "ADD") {
                        this.nbDisplayedSteps++;
                    }
                    this.displayedSteps = this.steps?.slice(
                        -this.nbDisplayedSteps,
                    );
                    this.checkWidth();
                });
    ...
    private _checkWidth() {
        ...
        if (listWidth > componentWidth) {
            ...
            this.nbDisplayedSteps > 0 &&
            this.nbDisplayedSteps < nbSteps
        ) {
            // else, try to expand
    ...
    private tryToExpand(): Promise<void> {
        if (
            this.nbDisplayedSteps >=
            this.steps.filter((s) => !s.disabled)?.length
        )
