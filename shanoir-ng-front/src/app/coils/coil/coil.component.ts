...
export class CoilComponent extends EntityComponent<Coil> {
    ...
    initView(): Promise<void> {
        return this.centerService.getAll().then((centers) => {
            this.coil.center = centers.find(
                (center) => center.id == this.coil.center.id,
            );
        });
    }
    ...
    initEdit(): Promise<void> {
        return this.centerService.getAll().then((centers) => {
            ...
            this.coil.center = this.centers.filter(
                (center) => center.id == this.coil.center.id,
            )[0];
            setTimeout(() => {
                this.updateManufList(this.coil.center);
            });
            this.coil.manufacturerModel = this.manufModels.filter(
                (manuf) => manuf.id == this.entity.manufacturerModel.id,
            )[0];
        });
    }
    ...
    initCreate(): Promise<void> {
        ...
        const centerPromise: Promise<void> = this.centerService
            .getAll()
            .then((centers) => {
                this.centers = centers;
            });
    ...
    buildForm(): UntypedFormGroup {
        const form: UntypedFormGroup = this.formBuilder.group({
            ...
            manufacturerModel: [
                {
                    value: this.coil.manufacturerModel,
                    disabled: !this.coil.center,
                },
                [Validators.required],
            ],
    ...
    updateManufList(center: Center): void {
        if (!this.form.get("manufacturerModel").disabled) {
            ...
            this.manufModels = center?.acquisitionEquipments?.map(
                (acqEq) => acqEq.manufacturerModel,
            );
            if (
                !this.coil.manufacturerModel ||
                !this.manufModels?.find(
                    (model) => model.id == this.coil.manufacturerModel.id,
                )
            ) {
