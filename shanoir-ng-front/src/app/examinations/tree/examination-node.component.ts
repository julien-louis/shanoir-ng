    ...
    implements OnChanges
{
    ...
    hasChildren(): boolean | "unknown" {
        if (!this.node.datasetAcquisitions && !this.node.extraDataFilePathList)
            return false;
        else if (
            this.node.datasetAcquisitions == "UNLOADED" ||
            this.node.extraDataFilePathList == "UNLOADED"
        )
            return "unknown";
        else
            return (
                (this.node.datasetAcquisitions &&
                    this.node.datasetAcquisitions.length > 0) ||
                (this.node.extraDataFilePathList &&
                    this.node.extraDataFilePathList.length > 0)
            );
    }
    ...
    firstOpen() {
        if (this.node.datasetAcquisitions == "UNLOADED") {
            this.loadDatasetAcquisitions().then(() =>
    ...
    loadDatasetAcquisitions(): Promise<void> {
        ...
        return this.datasetAcquisitionService
            .getAllForExamination(this.node.id)
            .then((dsAcqs) => {
                if (!dsAcqs) dsAcqs = [];
                dsAcqs = dsAcqs.filter((acq) => acq.type !== "Processed");
                this.node.datasetAcquisitions = dsAcqs.map((dsAcq) =>
                    DatasetAcquisitionNode.fromAcquisition(
                        dsAcq,
                        this.node,
                        this.node.canDelete,
    ...
    download() {
        ...
        this.massDownloadService
            .downloadAllByExaminationId(this.node.id, this.downloadState)
    ...
    onAcquisitionDelete(index: number) {
        (this.node.datasetAcquisitions as DatasetAcquisitionNode[]).splice(
            index,
            1,
        );
    }
